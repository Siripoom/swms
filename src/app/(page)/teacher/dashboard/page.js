...
const SubjectCard = ({ subject }) => (
  <Link href={`/teacher/subjects/${subject.id}`} key={subject.id} className="block group">
    <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 h-full flex flex-col overflow-hidden border border-gray-200">
      <div className="p-5">
        <div className="flex justify-between items-start mb-4">
          <div>
            <Title level={5} className="!mb-0 group-hover:text-indigo-600 transition-colors">
              {subject.subject_code}
            </Title>
            <Text type="secondary" className="text-xs truncate block max-w-[200px]">{subject.subject_name}</Text>
          </div>
        ...
        <div className="flex items-center text-gray-600 text-sm pt-4 border-t border-gray-100">
          ...
          <span>นักศึกษา:</span>
          <span className="font-semibold text-gray-800 ml-auto">{subject.enrollment_count || 0} คน</span>
        </div>
      </div>
      <div className="mt-auto bg-gray-50 px-5 py-3 border-t border-gray-200">
        <span className="text-sm font-semibold text-indigo-600 flex items-center justify-between">
          ดู Dashboard รายวิชา <RightOutlined className="transform group-hover:translate-x-1 transition-transform" />
        </span>
      </div>
    </div>
  </Link>
...
export default function TeacherDashboardPage() {
  ...
  if (!dashboardData && !loading) return <DashboardLayout><div className="p-6"><Empty description="ไม่พบข้อมูล Dashboard" /></div></DashboardLayout>;
      ...
      <div className="bg-white min-h-screen p-4 sm:p-6 lg:p-8">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-blue-500 text-white rounded-xl shadow-lg p-6">
            <Title level={2} className="!text-white">ยินดีต้อนรับ, {user?.name || 'อาจารย์'}</Title>
            <Text className="!text-white opacity-80">ภาพรวมรายวิชาทั้งหมดที่คุณรับผิดชอบ</Text>
          </div>
            ...
            <div className="space-y-8">
              {academicYears.length > 0 ? (
                academicYears.map(year => (
                  <div key={year}>
                    <Title level={3} className="flex items-center mb-4 pb-2 border-b-2 border-gray-100">
                      <CalendarOutlined className="mr-3 text-indigo-500" /> ปีการศึกษา {year}
                    </Title>
                ...
                <div className="bg-gray-50 rounded-lg p-10 text-center">
                  <Empty description={<Title level={5} type="secondary">คุณยังไม่มีรายวิชาที่รับผิดชอบ</Title>} />
                </div>
              )}
            </div>
          )}
