...
export default function TeacherAssignmentsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [assignRes, subjRes, tplRes] = await Promise.all([
      ...
      getSubjectsForTeacher(user.id),
      getAllTemplates(),
    ]);
    if (assignRes.success) setAssignments(assignRes.data);
    else message.error("ดึงข้อมูลภาระงานไม่สำเร็จ");
    if (subjRes.success) setSubjects(subjRes.data);
    else message.error("ดึงข้อมูลรายวิชาไม่สำเร็จ");
    if (tplRes.success) setTemplates(tplRes.data);
    else message.error("ดึงข้อมูลคลังไม่สำเร็จ");
    setLoading(false);
  }, [user]);
  ...
  const handleSubmit = async (values) => {
    ...
    const result =
      modalMode === "create"
        ? await createAssignmentAndDistribute(assignmentData)
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "type",
      responsive: ["lg"],
      render: (type) => (type === "individual" ? "เดี่ยว" : "กลุ่ม"),
    },
    {
      title: "วันที่มอบหมาย",
    ...
    },
    {
      title: "สถานะ",
      key: "status",
      render: (_, record) => getStatusTag(record.due_date),
    },
    {
      title: "ดำเนินการ",
      key: "actions",
      ...
      render: (_, record) => (
        <Space>
          ...
          <Tooltip title="ลบ">
            <Popconfirm
              title="คุณแน่ใจหรือไม่ที่จะลบภาระงานนี้?"
  ...
  return (
    <DashboardLayout title="จัดการภาระงาน">
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 mb-6 p-5 rounded-lg border border-gray-200">
            <div>
              <Title level={2} className="!mb-1">
                จัดการภาระงาน (การบ้าน)
              </Title>
              <Text type="secondary">
                เพิ่ม แก้ไข หรือลบงานที่มอบหมายให้นักศึกษา
              </Text>
            </div>
      ...
      <Modal
        title={
          <div className="flex items-center">
            ...
            {modalMode === "create" ? "เพิ่มภาระงานใหม่" : "แก้ไขภาระงาน"}{" "}
          </div>
        }
        open={isModalOpen}
      ...
      >
        ...
        >
          <Form.Item
            label="ดึงข้อมูลจากคลัง (ไม่บังคับ)"
            name="workload_template_id"
            tooltip="เลือกงานจากคลังเพื่อกรอกข้อมูลพื้นฐานให้อัตโนมัติ"
          >
            <Select
              placeholder="เลือกต้นแบบภาระงาน..."
              allowClear
            ...
            >
              {templates.map((tpl) => (
                <Option key={tpl.id} value={tpl.id}>
                  {`[${categoryTranslations[tpl.category] || tpl.category}] ${
                    tpl.name
                  } (${tpl.hours} ชม.)`}
                </Option>
              ))}
          ...
          <Row gutter={24}>
            <Col xs={24} lg={12}>
              <Form.Item
                ...
                rules={[{ required: true, message: "กรุณากรอกชื่อภาระงาน" }]}
              >
                <Input placeholder="เช่น รายงานเรื่อง..." />
              </Form.Item>
            </Col>
            <Col xs={24} lg={12}>
              <Form.Item
                ...
                >
                  {subjects.map((sub) => (
                    <Option
                      key={sub.id}
                      value={sub.id}
                    >{`${sub.subject_code} - ${sub.subject_name}`}</Option>
                  ))}
                </Select>
              </Form.Item>
            </Col>
          ...
          <Row gutter={24}>
            <Col xs={24} lg={12}>
              <Form.Item
                label="วันที่มอบหมาย"
                name="assigned_date"
                rules={[{ required: true, message: "กรุณาเลือกวันที่มอบหมาย" }]}
              >
                <DatePicker
                  style={{ width: "100%" }}
                  format="DD/MM/YYYY"
            ...
            <Col xs={24} lg={12}>
              <Form.Item
                label="กำหนดส่ง"
                name="due_date"
                rules={[{ required: true, message: "กรุณาเลือกกำหนดส่ง" }]}
              >
                <DatePicker
                  style={{ width: "100%" }}
                  format="DD/MM/YYYY"
          ...
          <Form.Item label="รายละเอียด (ไม่บังคับ)" name="description">
            <Input.TextArea
              rows={4}
              placeholder="อธิบายรายละเอียดของงานที่มอบหมาย..."
            />
          </Form.Item>
        </Form>
      </Modal>
