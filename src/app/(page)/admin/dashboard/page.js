...
import { useCache, useAsyncOperation } from "@/hooks/useCache";
import {
...
const ChartCard = ({
  ...
  loading,
  type = "bar",
  emptyText = "ไม่มีข้อมูลสำหรับแสดงผล",
}) => (
  <Card
    title={
...
export default function AdminDashboardPage() {
  ...
  const [assignmentVsWorkloadChart, setAssignmentVsWorkloadChart] =
    useState(null);
  ...
  useEffect(() => {
    const fetchFilterOptions = async () => {
      ...
      if (yearRes.success && yearRes.data.length > 0) {
        const options = yearRes.data.map((year) => ({
          value: year,
        ...
        setYearOptions([
          { value: currentYear, label: `ปีการศึกษา ${currentYear}` },
        ]);
        if (!selectedYear) {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (yearLevelRes.success && yearLevelRes.data.length > 0) {
      ...
      const yearLevels = [
        ...new Set(yearLevelRes.data.map((item) => item.year_level)),
      ].sort();
      const series = categories.map((category) => ({
        name: categoryTranslations[category],
        data: yearLevels.map((year) => {
          const item = yearLevelRes.data.find(
            (d) => d.year_level === year && d.category === category
          );
          const avgHours =
            item && item.student_count > 0
              ? item.total_hours / item.student_count
              : 0;
      ...
      setYearLevelChart({
        ...
        options: {
          chart: { stacked: true },
          xaxis: { categories: yearLevels.map((y) => `ปี ${y}`) },
          yaxis: { title: { text: "ชม.เฉลี่ย/คน" } },
        },
      });
    } else {
      setYearLevelChart(null);
    }
    ...
    if (subjectRes.success && subjectRes.data.length > 0) {
      ...
      setSubjectChart({
        series: [
          {
            name: "ชม. ที่คาดหวัง",
            data: topSubjects.map((s) => parseFloat(s.avg_hours.toFixed(1))),
          },
        ],
        options: {
          ...
          xaxis: { categories: topSubjects.map((s) => s.subject_code) },
        },
      });
    } else {
      setSubjectChart(null);
    }
    ...
    if (distributionRes.success && distributionRes.data.length > 0) {
      ...
      setDistributionChart({
        series: [
          {
            name: "ค่าต่ำสุด",
            data: distributionRes.data.map((d) =>
              parseFloat(d.min_hours.toFixed(1))
            ),
          },
          {
            name: "ค่าเฉลี่ย",
            data: distributionRes.data.map((d) =>
              parseFloat(d.avg_hours.toFixed(1))
            ),
          },
          {
            name: "ค่าสูงสุด",
            data: distributionRes.data.map((d) =>
              parseFloat(d.max_hours.toFixed(1))
            ),
          },
        ],
        options: {
    ...
    if (creditRes.success && creditRes.data.length > 0) {
      setCreditVsWorkloadChart({
        ...
        options: {
          ...
          xaxis: { title: { text: "จำนวนหน่วยกิต" } },
          yaxis: { title: { text: "ชม. ที่คาดหวัง" } },
        },
      });
    } else {
    ...
    if (
      assignmentVsWorkloadRes.success &&
      assignmentVsWorkloadRes.data.length > 0
    ) {
      setAssignmentVsWorkloadChart({
        series: [
          {
            name: "ชม. ที่คาดหวังเฉลี่ย",
            data: assignmentVsWorkloadRes.data.map((d) => [
              d.assignment_count,
              parseFloat(d.avg_hours.toFixed(1)),
            ]),
          },
    ...
    if (proportionRes.success && proportionRes.data.length > 0) {
      const categories = proportionRes.data.map(
        (p) => categoryTranslations[p.category] || p.category
      );
      const series = [
        {
          name: "ตามแผน (%)",
          data: proportionRes.data.map((p) => p.target_percentage),
        },
        {
          name: "ตามจริง (%)",
          data: proportionRes.data.map((p) =>
            parseFloat(p.actual_percentage.toFixed(1))
          ),
        },
      ];
      setProportionChart({
  ...
  const summaryStats = [
    {
      title: "ชม. ภาระงานเฉลี่ย",
      value: Number(summaryData?.avg_total_hours || 0).toFixed(1),
      unit: "ชม./คน",
      icon: <TeamOutlined />,
      color: "#3b82f6",
    },
    {
      title: "ชม. วิชาการเฉลี่ย",
      value: Number(summaryData?.avg_academic_hours || 0).toFixed(1),
      unit: "ชม./คน",
      icon: <BookOutlined />,
      color: "#8b5cf6",
    },
    {
      title: "ชม. กิจการ นศ. เฉลี่ย",
      value: Number(summaryData?.avg_student_affairs_hours || 0).toFixed(1),
      unit: "ชม./คน",
      icon: <ApartmentOutlined />,
      color: "#16a34a",
    },
    {
      title: "ชิ้นงานเฉลี่ย/วิชา",
      value: Number(summaryData?.avg_assignment_count_per_subject || 0).toFixed(
        1
      ),
      unit: "ชิ้น/วิชา",
      icon: <RiseOutlined />,
  ...
  const chartCards = [
    {
      title: "ภาพรวมภาระงานตามชั้นปี (กราฟ 1)",
      icon: <BarChartOutlined />,
      chartData: yearLevelChart,
      type: "bar",
    },
    {
      title: "10 อันดับรายวิชาภาระงานสูงสุด (กราฟ 2)",
      icon: <AreaChartOutlined />,
      chartData: subjectChart,
      type: "bar",
    },
    {
      title: "การกระจายตัวของภาระงาน (กราฟ 3)",
      icon: <BarChartOutlined />,
      chartData: distributionChart,
      type: "bar",
    },
    {
      title: "หน่วยกิต vs ภาระงาน (กราฟ 4)",
      icon: <DotChartOutlined />,
      chartData: creditVsWorkloadChart,
      type: "scatter",
    },
    {
      title: "ชิ้นงาน vs ชั่วโมงทำงาน (กราฟ 5)",
      icon: <DotChartOutlined />,
      chartData: assignmentVsWorkloadChart,
      type: "scatter",
    },
    {
      title: "สัดส่วนภาระงาน: แผน vs จริง (กราฟ 6)",
      icon: <PieChartOutlined />,
      chartData: proportionChart,
      type: "bar",
    },
  ];
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="flex flex-wrap justify-between items-center gap-4">
            ...
            <Space
              direction="vertical"
              align="end"
              className="w-full sm:w-auto sm:flex-row"
            >
              <Select
                value={selectedYear}
