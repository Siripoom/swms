...
// 2. ดึงรายวิชาที่อาจารย์ได้รับมอบหมายเท่านั้น
export async function getSubjectsForTeacher(teacherId) {
  if (!teacherId) return { success: false, error: "Teacher ID is required" };
  const { data, error } = await supabase
...
export async function createAssignmentAndDistribute(assignmentData) {
  ...
  if (insertWorkloadError)
    return { success: false, error: insertWorkloadError.message };
